...
document.addEventListener('DOMContentLoaded', () => {
  ...
  toggles.forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      // Stop other preview if one is playing
      if (currentAudio && currentAudio !== audioEl) {
        currentAudio.pause();
        if (currentBtn) {
          currentBtn.textContent = '▶ Preview (0:30)';
          currentBtn.classList.remove('is-playing', 'is-loading');
        }
      }
  ...
  // Reset button when a preview ends
  document.querySelectorAll('audio').forEach(a => {
    a.addEventListener('ended', () => {
      if (currentAudio === a && currentBtn) {
        currentBtn.textContent = '▶ Preview (0:30)';
        currentBtn.classList.remove('is-playing', 'is-loading');
        currentAudio = null;
        currentBtn = null;
